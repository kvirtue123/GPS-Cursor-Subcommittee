...
![Agent Script Blocks](../../../../../media/agent-script/agent-script-blocks2.svg '{"class": "image-sm"}')
...
```sfdocs-code {"lang":"agentscript", "title": "model_config"}
   model_config:
...
- **Can** deterministically call an [action](reference/ascript-ref-actions.md).

### `model_config` Example

In this example, the agent router uses the EinsteinHyperClassifier model.

```sfdocs-code {"lang":"agentscript", "title": "Agent Router Subagent With EinsteinHyperClassifier Model "}
start_agent agent_router:
    description: "Welcome the user and determine the appropriate subagent based on user input"
